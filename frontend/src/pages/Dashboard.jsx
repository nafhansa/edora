  ...
  const refreshData = () => {
    ...
    Promise.all([fetchStats, fetchPatients])
      .then(([statsRes, patientsRes]) => {
        ...
        setPatients(pts)
        // jika belum ada pasien terpilih, otomatis pilih yang pertama agar panel monitoring menampilkan data
  ...
  useEffect(() => {
    ...
    if (selectedPatient) {
      ...
      const fetchLatestRecords = () => {
        axios.get(`/api/v1/patients/${selectedPatient.id}/medical_records`)
          .then(res => {
            setRecords(res.data || []);
